...
const FARM_GUIDES_KEY_CLOSED = 'SHOW_FARM_GUIDES_KEY';

export const FarmGuides: FC = () => {
  const { valBySize } = useDevice();

  const isFarmGuidesShowed = !localStorageManager.get<boolean>(
    FARM_GUIDES_KEY_CLOSED,
  );

  const closeGuide = () =>
    localStorageManager.set(FARM_GUIDES_KEY_CLOSED, true);
              ...
              >
                <Flex.Item display="flex" col marginBottom={valBySize(2, 0)}>
                  <Flex.Item marginBottom={2}>
                    <StyledTitle level={4}>
                      <Trans>
                        Earn more tokens from your liquidity positions
                      </Trans>
                    </StyledTitle>
                  </Flex.Item>
                  <StyledBodyText>
