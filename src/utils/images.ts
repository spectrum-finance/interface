// ICONS
export const ARROW_DOWN = '/img/icons/arrow-down-short.svg#icon';
export const ICON_COPY = '/img/icons/copy-dark.png';
export const ICON_EXPLORE = '/img/icons/explore.png';
export const ICON_POINTS = '/img/icons/points.png';
export const CHEVRON_DOWN = '/img/icons/chevron-down.svg#icon';
export const EXCLAMATION = '/img/icons/exclamation.svg#icon';
export const COIN = '/img/icons/coin.svg#icon';
export const SEARCH = '/img/icons/search.svg#icon';
export const VERIFIED = '/img/icons/verified.png';
export const BANK = '/img/icons/bank.svg#icon';
export const WARNING = '/img/icons/exclamation.png';
export const X = '/img/icons/x.svg#icon';
export const X_CIRCLE = '/img/icons/x-circle.svg#icon';
export const CHECK_CIRCLE = '/img/icons/check-circle.svg#icon';
export const ARROW_RIGHT = '/img/icons/arrow-right.svg#icon';
export const PLUS = '/img/icons/plus.svg#icon';

// TOKENS

export const TOKEN_ADA = '/img/tokens/ADA.png';
export const TOKEN_TEDY = '/img/tokens/TEDY.png';
